...
# 2 "main.c" 2
# 1 "../StackArray/StackArray.h" 1

# 1 "../headers/aliases.h" 1
...
typedef enum { OK, OUT_OF_RANGE, NULL_POINTER } Error;
# 4 "../StackArray/StackArray.h" 2
...
typedef struct u32Sarr_t {
  u32 *data;
  u32 len;
  u32 cap;
  u32Return (*get)(struct u32Sarr_t *self, u32 idx);
  Error (*set)(struct u32Sarr_t *self, u32 idx, u32 data);
  Error (*push)(struct u32Sarr_t *self, u32 data);
  Error (*pop)(struct u32Sarr_t *self);
} u32Sarr;
u32Return u32Get(struct u32Sarr_t *self, u32 idx) {
...
# 5 "main.c"
    ...
    return (u32Return){.err = OUT_OF_RANGE};
  } else if (self ==
# 5 "main.c" 3 4
             ((void *)0)
# 5 "main.c"
...
# 5 "main.c"
    ...
    \ return (u32RETURN) { .err = NULL_POINTER; }
  } else {
    return (u32Return){.val = self->data[idx], .err = OK};
  }
}
Error u32Set(struct u32Sarr_t *self, u32 idx, u32 data) {
  if (idx < 0 || idx >= self->len) {
    raise(
# 5 "main.c" 3 4
        5
...
typedef struct i32Sarr_t {
  i32 *data;
  u32 len;
  u32 cap;
  i32Return (*get)(struct i32Sarr_t *self, u32 idx);
  Error (*set)(struct i32Sarr_t *self, u32 idx, i32 data);
  Error (*push)(struct i32Sarr_t *self, i32 data);
  Error (*pop)(struct i32Sarr_t *self);
} i32Sarr;
i32Return i32Get(struct i32Sarr_t *self, u32 idx) {
  if (idx < 0 || idx >= self->len) {
    raise(
# 6 "main.c" 3 4
        5
# 6 "main.c"
    ...
    return (i32Return){.err = OUT_OF_RANGE};
  } else if (self ==
# 6 "main.c" 3 4
             ((void *)0)
# 6 "main.c"
...
# 6 "main.c"
    ...
    \ return (i32RETURN) { .err = NULL_POINTER; }
  } else {
    return (i32Return){.val = self->data[idx], .err = OK};
  }
}


Error i32Set(struct i32Sarr_t *self, u32 idx, i32 data) {
  if (idx < 0 || idx >= self->len) {
    raise(
# 6 "main.c" 3 4
        5
...
int main() {
  printf("no siema\n");
  u32 myarrData[4];
  u32Sarr myarr = {.data = myarrData, .len = 2, .cap = 4, .get = u32Get};
  ...
  i32 intarrData[4];
  i32Sarr intarr = {.data = intarrData, .len = 2, .cap = 4, .get = i32Get};
